...
<body>
	...
	<section>
		...
		<p>While this continues to be used for single, stand-alone scripts, i.e. those with no dependencies, it does not function well with the larger libraries you page may rely on. For instance, using either the <span class="monospace">async</span> or <span class="monospace">defer</span> properties in conjunction with your jQuery will cause an error if later scripts run before it is finished loading. These errors look something like <span class="monospace color-flatui-pomegranate">$ is not defined</span> or <span class="monospace color-flatui-pomegranate">jquery is not defined</span>.</p>
	...
	<section>
		...
		<p>One of the many reasons we like <a class="color-flatui-belize-hole" href="https://www.jsdelivr.com/">jsDelivr</a> is because it provides a unique file combiner, which eradicates extraneous requests which, especially with smaller files, can add considerable latency (literally) to your page.</p>
	...
	<section>
		...
		<p>According to <a class="color-flatui-belize-hole" href="https://www.cdnperf.com/">CDNperf</a>, jsDelivr offers one of the lowest latencies of any CDN. Latency is the time a user's computer takes to find a site, so a lower latency means starting to load sooner. Stay away from Yandex unless your site's users are local to it, and you should be fine.</p>
	</section>
	<hr id="footer">
	<footer>
